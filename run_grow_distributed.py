...
def train(args, train_dataset, eval_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    residual_weights = [".residual_weight", ".residual_bias"]  # 为什么多出来residual参数？这里不是应该只优化LiGO operator的参数吗？
    ...
    if args.gpu == 0:
        print("Optimized with lr %f, steps %d, warmup steps %d, and use beta, epsilon %0.8f." % (
            args.learning_rate, t_total, args.warmup_steps, optimizer.defaults['eps']), optimizer.defaults['betas'])
...
def setup(gpu, args):
    ...
    # Initialization of LiGO operator, 啥意思啊，转换矩阵的初始化方式？？？
    ...
    model = create_ligo_from_model(model, args)  # 跟我之前的实现方案差不多。。
