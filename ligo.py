...
class FusedWidthParams(nn.Module):
    ...
    def __init__(self, small_dim, large_dim, learnable=False, init_scheme='rand', init_noise=0.03):
        ...
        if large_dim - small_dim > 0:
            if learnable:  # 这我就不理解了，难道是copy小模型参数，仅对大模型增加的参数进行可学习初始化吗？
    ...
    def reset_parameters(self):
        if self.coeffs_weight is not None:
            if self.init_scheme in ['rand', 'rand_softmax']:
                normalized_uniform_init(self.coeffs_weight, self.init_scheme)  # 如果0初始化会怎样？看看最终结果？
...
class FusedLinear(nn.Module):
    ...
    def __init__(self, model, module_name, in_features, out_features, layer_index=-1, init_scheme_depth='rand', init_noise_depth=0.03, learn_depth=True,
        ...
        self.get_weights = lambda: getattr(model, K2N(module_name) + '_weight')  # 这个K2N dict包含了小模型的参数？
        self.get_bias = lambda: getattr(model, K2N(module_name) + '_bias', None)

        self.bias = (self.get_bias() is not None)

        self.residual = residual  # 这是什么玩意儿？
    ...
    def get_params(self):
        ...
        if hasattr(self, 'get_depth_coeffs'):  # 对较大模型的某一层，参数为小模型的参数乘上转换系数？
        ...
        in_dim_expand, out_dim_expand = self.get_width_coeffs()  # 这怎么是先加深，再加宽啊？和文中相反？
...
def create_ligo_from_model(model_large, args, source_model=None):
    ...
    M2N = lambda m: dict_M2N[id(m)]  # module to name长什么样子啊？
    ...
    for name, param in dict_model_small.items():
        if not is_encoder_layer(name):
            if args.tune_small_model:
                ...
                model_large.register_buffer(K2N(name), param, persistent=True)  # 这个K2N是干啥啊，给large model的参数名打标？
    ...
    for name, param in enc_layers[0].named_parameters():
        weight_list = []
        for l, _ in enumerate(enc_layers):  # 对每一个block；这里有没有考虑bias啊？
    ...
    for i, l_large in enumerate(large_layers):
        if args.fuse_tie_param:  # 为什么name中带有fuse？fuse指的是reuse？这里注册转换矩阵。。
            # 这里的key是一个head，还是所有head一起？
            # 在深度方向上，为什么对每一层加入宽度转换系数？
        ...
        for name in ['query', 'key', 'value']:  # 利用old weights和转换矩阵，得到大模型的初始化参数
